...
import org.weixiao.common.Constant;
...
import org.weixiao.util.StringArrayUtil;
...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        isAuth = true;
        BufferedReader reader = new BufferedReader(new FileReader(aofFilePath));
        String line;
        while ((line = reader.readLine()) != null) {
            executeCommand(database, line, null);
        }
        BufferedWriter writer = new BufferedWriter(new FileWriter(aofFilePath));
        writer.write("");
        writer.close();
        isAuth = false;
        ...
        do {
            ...
            executeCommand(database, input, aofFilePath);
        } while (!exit);
        scanner.close();
    }
    ...
    private static void executeCommand(RedisDatabase database, String input, String aofFilePath) {
        ...
        try {
            if (Objects.equals(command, "auth")) {
                if (!checkArgumentsNum(arguments, 1)) return;
                ...
                if (StringArrayUtil.find(Constant.SIDE_EFFECT_COMMAND, command) != -1) {
                    if (aofFilePath != null && !aofFilePath.isEmpty()) {
                        BufferedWriter writer = new BufferedWriter(new FileWriter(aofFilePath, true));
                        writer.append(input);
                        writer.append("\n");
                        writer.close();
                    }
